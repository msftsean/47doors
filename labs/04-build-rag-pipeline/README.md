...
- What categories and topics they cover
...
**Task:** Complete the index schema in `start/index_schema.json`.
...
**Task:** Complete the indexer in `start/indexer.py` and run it to populate your index.
...
az search document search --service-name <your-service> \
    --index-name kb-index --search "*" --top 5
```
...
|-------------|------------------|
| Azure AI Search Index | Index created with vector and keyword fields configured |
| Indexed KB Articles | All KB articles indexed with embeddings |
...
- **Solution:** Verify vector dimensions match your embedding model (1536 for text-embedding-ada-002)
...
1. [ ] Azure AI Search service is running and accessible
