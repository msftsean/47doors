...
async def test_query_agent():
    ...
    for message, expected_intent in test_cases:
        ...
        status = "✅" if expected_intent in result.intent.value.lower() or result.intent.value.lower() in expected_intent else "⚠️"
...
async def test_full_pipeline():
    ...
    for i, message in enumerate(conversation, 1):
        ...
        print(f"Confidence: {result.confidence:.2f}")
        if result.sources:
            print(f"Sources: {[s.get('title', s.get('id', 'unknown'))[:30] for s in result.sources[:2]]}")
