...
async def submit_query(
    *,
    ...
    if router_agent.needs_clarification(query_result, session.clarification_attempts):
        ...
        # Deduplicate possible intents to avoid nonsensical questions
        possible_intents = list(dict.fromkeys([query_result.intent, "login_issues", "password_reset"]))
...
async def health_check(
    ...
    # Realtime API (voice) health check
    if settings.voice_enabled:
        if settings.use_mock_services:
            services["realtime_api"] = ServiceHealth(
                status="up",
                latency_ms=0,
                error=None,
            )
        elif settings.azure_openai_realtime_deployment:
            services["realtime_api"] = ServiceHealth(
                status="up",
                latency_ms=None,
                error=None,
            ...
            services["realtime_api"] = ServiceHealth(
                status="down",
                latency_ms=None,
                error="No realtime deployment configured",
            )
    else:
        services["realtime_api"] = ServiceHealth(
            status="down",
