...
class Settings(BaseSettings):
    ...
    voice_enabled: bool = Field(
        default=True,
        description="Enable voice interaction feature"
    )
    azure_openai_realtime_endpoint: str = Field(
        default="",
        description="Azure OpenAI Realtime API endpoint (if different from main endpoint)"
    )
    azure_openai_realtime_api_key: str = Field(
        default="",
        description="Azure OpenAI Realtime API key (if different from main key)"
    )
    azure_openai_realtime_deployment: str = Field(
        default="gpt-realtime",
        description="Azure OpenAI Realtime API deployment name"
    )
    ...
    realtime_voice: str = Field(
        default="alloy",
        description="Voice name for the Realtime API"
    )
    realtime_vad_threshold: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="Voice activity detection silence threshold"
    )
