...
class MockLLMService(LLMServiceInterface):
    ...
    async def classify_intent(
        ...
        """Classify intent using pattern matching (system_prompt ignored in mock)."""
        # Detect PII
    ...
    async def generate_clarification_question(
        ...
        """Generate a clarification question (system_prompt ignored in mock)."""
        ...
        for intent in possible_intents[:5]:  # Check up to 5 intents
            desc = intent_descriptions.get(intent, intent.replace("_", " "))
            # Deduplicate descriptions to avoid nonsensical questions
            if desc not in seen_descriptions:
                options.append(desc)
                seen_descriptions.add(desc)
            if len(options) >= 3:  # Limit to 3 unique options
                break
    ...
    async def generate_response_message(
        ...
        """Generate a user-friendly response message with self-service info (system_prompt ignored in mock)."""
        dept_names = {
